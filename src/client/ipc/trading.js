...
import {REMOVE_ARTWORK_FROM_SELL_LOADING, SET_SELL_TICKET_ERROR, SET_SELL_TICKET_MSG} from "../actionTypes";
...
ipcRenderer.on('sellArtworkResponse', (event, data) => {
  switch (data.status) {
    case constants.RESPONSE_STATUS_ERROR:
      ...
      store.dispatch({type: SET_SELL_TICKET_ERROR, error: data.error});
      break;
    case constants.RESPONSE_STATUS_OK:
      store.dispatch({type: REMOVE_ARTWORK_FROM_SELL_LOADING, image_hash: data.image_hash});
      store.dispatch({type: SET_SELL_TICKET_MSG, message: `Sell ticket created, txid: ${data.txid}`});
